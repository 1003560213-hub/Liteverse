...
7. After the user explicitly chooses one option—or explicitly delegates that choice to Codex—run `scripts/apply-partition-choice.mjs` with that option ID, `--confirmed-by-user`, a confirmation note, and an ISO decision time. For delegation, the note must identify it as a `delegated choice` and record the recommendation basis. The script rechecks `baseRevision`, the exact source snapshot, and current/corpus artifact fingerprints, appends the choice, assigns persistent unused-first nebula assets, and deterministically regenerates finite category centers and paper positions. New or changed centers prefer low-occupancy footprints in the hash-pinned default background, softly minimize nebula overlap in the default camera projection, and retain three-dimensional depth; overlap remains allowed when the view is crowded. The script writes an ordinary unstaged snapshot. Never infer a choice from silence or a prior preference. Use `--rebuild-selected` only to regenerate the exact already-recorded selection; it adds no decision and cannot change the option.
